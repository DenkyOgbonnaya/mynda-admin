...
import { updateEmployer } from "services/user.service";

import { IDENTITY_DOCUMENT_TYPES } from "shared";
import { Employer, EmployerCreate } from "types/employer.interface";
...
export default function EmployerForm({ onSubmit, data, onCancel }: Props) {
  const {
    state: input,
    onChangeByNameValue,
    onChange,
    setState,
  } = useInputChange<Partial<EmployerCreate>>(
    data || {
      document: null,
      docType: "",
      docNumber: "",
  ...
  const { isLoading, mutate } = useMutation(
    async (cred: EmployerCreate) => updateEmployer(data?.user!, cred),
    {
      onSuccess(res) {
        queryClient.invalidateQueries(["profile", data?.user!]);
